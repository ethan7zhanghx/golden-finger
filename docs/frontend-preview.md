...
进入项目后，进入三栏式工作台界面：左侧步骤导航、中央全高编辑区、右侧 AI 助手面板。编辑区采用「白纸卡片」设计，填满整个可视视口，让编剧专注于创作。
...
| 区域 | 宽度 | 功能 |
|------|------|------|
| 左侧导航 | 224px | 16步骤树状导航，显示完成状态 |
| 中央编辑区 | flex-1 (全宽) | 全高纸质卡片编辑器，填满视口 |
...
富文本步骤（如开头写作、台词写作、剧本介绍）使用 TipTap 编辑器，支持格式工具栏。编辑器铺满全屏高度，提供沉浸式写作体验。

![工作台 - 开头写作](screenshots/04-editor-richtext.png)

**富文本功能：**
- 工具栏：加粗、斜体、H1/H2 标题、列表/编号列表
- 实时自动保存（2秒防抖）
- 底部状态指示：「已保存」/ 「未保存的更改…」
...
基于用户反馈（编辑器太小、视觉品质对标 Laper），完成以下改进：

| 改进项 | 改进前 | 改进后 |
|--------|--------|--------|
| 编辑器高度 | min-h-[300px]，约300px | 全视口高度（calc(100vh-16rem)），填满屏幕 |
| 中央区域宽度限制 | max-w-4xl（约896px） | 无限制，flex-1 占满全宽 |
| 编辑器容器设计 | 普通带边框 div | 白纸卡片（shadow-sm + rounded-xl） |
| 左侧导航宽度 | 256px | 224px，更紧凑 |
| TipTap 工具栏 | 4个简单按钮 | 分隔线 + H1/H2 + 列表/编号列表 |
| React 无限循环 Bug | 仍存在 | 已修复（EMPTY_CANDIDATES 常量） |
| TipTap SSR 警告 | 存在 | 已修复（immediatelyRender: false） |
